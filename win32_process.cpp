...
int Win32_pty::KillAllJobsInBkgndProcessQueue() {
   ...
   if( IsThreadActive() && Confirm( "Kill background %s process (PID=%ld)?", d_pfLogBuf->Name(), d_processInfo.dwProcessId ) ) {
      PCChar msg;
...
int InternalShellJobExecutor::KillAllJobsInBkgndProcessQueue() {
   ...
   if(   INVALID_dwProcessId != d_processInfo.dwProcessId
      && Confirm( "Kill background %s process (PID=%ld)?", d_pfLogBuf->Name(), d_processInfo.dwProcessId )
