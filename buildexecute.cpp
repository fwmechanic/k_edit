...
std::string StreamArgToString( PFBUF pfb, Rect stream ) {
   ...
   if( !IsStringBlank( rl ) ) {
      ...
      if( stream.flMin.col < lastCol ) {
         ...
         const auto nonb( FirstNonBlankOrEnd( rl, 0 ) );
   ...
   if( !IsStringBlank( rl ) ) {
      ...
      const auto nonb( FirstNonBlankOrEnd( rl, 0 ) );        0 && DBG( "nonb=%" PR_BSRSIZET "u maxIx=%" PR_BSRSIZET "u", nonb, maxIx );
      if( nonb < maxIx ) {
   ...
   while(1) { //******************************************************************
      ...
      if( pCmd->d_argData.eka.EdKcEnum == EdKC_tab ) { // 20100222 hack: look at EdKcEnum since new tab key assignment is to a Lua function
         ...
         if( g_CurFBuf() && stb.length() == xCursor ) {
            ...
            stTmp = g_CurFBuf()->GetLineSeg( g_CursorLine(), xx, xx );   0 && DBG( "%d='%" PR_BSR "'", xx, BSR(stTmp) );
            if( !stTmp.empty() ) {
