...
STATIC_FXN int SaveWinViewsToStateFile( FILE *fout, PWin pWin ) {
   ...
   // DO!  NOT!  DESTROY  THE  VIEW  LIST  IN  THIS  FUNCTION  !!!
   ...
   DLINKC_FIRST_TO_LASTA( pWin->ViewHd, dlinkViewsOfWindow, pv ) {
      if( g_iMaxfilehist == 0 || iFilesSaved < g_iMaxfilehist ) {
         if( pv->FBuf()->FnmIsDiskWritable() && !pv->FBuf()->ToForgetOnExit() ) {
            pv->Write( fout );
            ++iFilesSaved;
            }
         }
...
void WriteStateFile() {
   ...
   //
   // pro: choosing info to save based upon recentless of visit is much better
   //      than the old hacky algo (above).
   //
