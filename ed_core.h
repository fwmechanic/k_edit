...
public:
   ...
   PCChar vFmtStr( PCChar format, va_list val ) {
      const size_t olen( 1+_vsnprintf( nullptr , 0   , format, val ) );
      const auto rv( wresize( olen ) );
                           _vsnprintf( rv      , olen, format, val );
      return rv;
      }
...
class LineColorsClipped;  STD_TYPEDEFS( LineColorsClipped )
...
class                            FileTypeSettings;  STD_TYPEDEFS( FileTypeSettings )
...
public:
   COL            getLineRaw(       std::string &st, LINE yLine ) const;
   COL            getLineRaw(             PXbuf pXb, LINE yLine ) const { return getLine_( pXb, yLine, 0   ); }
