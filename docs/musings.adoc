...
essentially the same thing as the packaging of the then-current Nuwen MinGW
GCC toolset (a historical predecessor of today's MSYS2 UCRT64 toolchain).
...
1. Switch to compiling for Windows using GCC.  The original port used the now-retired Nuwen MinGW GCC distribution (thanks STL!!!); the current Windows build uses MinGW-w64 GCC supplied by MSYS2 UCRT64.  Once the original port was accomplished, it set the stage for...
