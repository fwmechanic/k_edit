 ...
 * *Hosted under Windows Terminal* (the Win11 default): on startup k.exe detects the `WT_SESSION` environment variable and transparently re-launches itself under `conhost.exe`, then waits for that instance and propagates its exit code.  This gets back all the legacy-console behaviors K depends on (no Ctrl+V hijacking by WT, native mouse-wheel scrolling, full-screen console resize, honor of per-EXE "Use legacy console" / QuickEdit registry settings).  Set environment variable `K_NO_CONHOST_RELAUNCH=1` to disable (e.g. for debugging k.exe under WT).
