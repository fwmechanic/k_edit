...
### Terminal Input Backends (Linux)

On Linux K supports two input backends:

- ncurses (default) — legacy escape decoding using terminfo.
- Kitty Keyboard Protocol (KKP) — CSI u decoding for precise key reporting.

When running inside a Kitty terminal, K auto‑detects KKP and enables it. You can also select explicitly via the command line:

- `-I ncurses` — force the ncurses backend
- `-I kitty` — force the Kitty KKP backend

With KKP enabled K requests the following progressive enhancements from Kitty: disambiguate, report_all_keys, and report_text. This makes printable keys, function keys, and numeric keypad operators report as CSI u events with modifiers, allowing robust decoding of combinations such as `ctrl+alt+num*`.

See also: `doclinks/kitty_keyboard_protocol.txt` for upstream KKP documentation links.
 ...
 * `GCC` >= 12 (for adequate C++23 support)
 ...
 * The primary Windows build uses the MinGW-w64 GCC toolchain supplied by https://www.msys2.org/[MSYS2] UCRT64.  Before April 2026, K used the now-retired http://nuwen.net/mingw.html[Nuwen MinGW] distribution; that historical toolchain was replaced after it dropped PCRE2 in late 2025.
 * Install https://www.msys2.org/[MSYS2] (default location `C:\msys64`), launch the *MSYS2 UCRT64* shell (Start menu: "MSYS2 UCRT64"), and install the toolchain plus project dependencies:
+
----
pacman -Syu                            # update base packages; re-run if prompted to re-launch the shell
pacman -S --needed \
  ...
  make
# optional: mingw-w64-ucrt-x86_64-gdb  (for crash debugging)
----
 * Build: from the UCRT64 shell run `make -j`.  Alternatively, from any other bash shell (e.g. Git-for-Windows bash), invoke `./msys2_build_run.sh` in this repo; it re-execs itself under MSYS2 UCRT64 bash and sets `TMP`/`TEMP`/`TMPDIR` to a Windows-style path (native `gcc.exe` doesn't grok MSYS-style `/tmp`).

 * FYI: MSYS2 UCRT64 binaries dynamically link to `ucrtbase.dll` and its `api-ms-win-crt-*.dll` ApiSet stubs (shipped with every Windows 10+ install; the modern successor to the ancient `msvcrt.dll`).  `libwinpthread-1.dll` is an additional runtime dep introduced by GCC's POSIX threading model; the Makefile locates it beside the active C++ compiler and copies it next to `k.exe` at build time (and includes it in `make rls`).  Full-static linking of winpthread is not viable in current MSYS2 packaging because libstdc++'s `thread.o` references `nanosleep64`, which exists only in the DLL.
 ...
 * ensure that GCC >= 12 is available to provide for C++23 support; on Ubuntu 22.04 install `g++-12` via apt (the default is GCC 11).
 ...
 * *Hosted under Windows Terminal* (the Win11 default terminal): on startup k.exe checks the class name of its console window via `GetClassName( GetConsoleWindow() )`.  Real conhost uses `ConsoleWindowClass`; WT/ConPTY (and any other VT emulator using ConPTY) uses `PseudoConsoleWindow`.  If the class isn't `ConsoleWindowClass`, k.exe transparently re-launches itself under `conhost.exe`, waits for that instance, and propagates its exit code.  This gets back all the legacy-console behaviors K depends on (no Ctrl+V hijacking, native mouse-wheel scrolling, full-screen console resize, honor of per-EXE "Use legacy console" / QuickEdit registry settings).  Class-name is used rather than other signals (`WT_SESSION` env var, `IsWindowVisible`, window rect, owner-process name) because each of those has corner cases that give wrong answers under Win11's "default-terminal" routing, by which WT can host consoles spawned by unrelated parents like Total Commander.  Set environment variable `K_NO_CONHOST_RELAUNCH=1` to disable the relaunch (e.g. for debugging k.exe under WT).
...
Per http://blogs.msdn.com/b/ricom/archive/2009/06/10/visual-studio-why-is-there-no-64-bit-version.aspx[Visual-Studio-Why-is-there-No-64-bit-Version], a 32-bit K might be more efficient unless editing files larger than 2GB.  Historically, however, K's Windows build became 64-bit-only when the former Nuwen MinGW distribution stopped providing 32-bit releases.  The current MSYS2 UCRT64/MinGW-w64 build remains 64-bit-only.  Linux in 2014+ is likewise almost universally 64-bit, and 64-bit Linux K has no known anomalies.

Historical toolchain note — 2026/04: Migrated the Windows build from the former http://nuwen.net/mingw.html[Nuwen MinGW] distribution to https://www.msys2.org/[MSYS2] UCRT64.  Nuwen dropped PCRE2 in late 2025, and since PCRE2 is central to K's regex functionality, continuing with that distribution was untenable.  The switch to UCRT64 also moved K from the 1990s-era `msvcrt.dll` to the modern `ucrtbase.dll` (UCRT, shipped with every Windows 10+ install), with a new runtime dependency on `libwinpthread-1.dll` from GCC's POSIX threading model.
