 ...
 * *Hosted under Windows Terminal* (the Win11 default terminal): on startup k.exe tests whether `IsWindowVisible( GetConsoleWindow() )` is true.  Real conhost creates a visible console window; ConPTY (Windows Terminal, OpenConsole, any other VT emulator) attaches an invisible pseudo-window to the client process.  If invisible, k.exe transparently re-launches itself under `conhost.exe`, waits for that instance, and propagates its exit code.  This gets back all the legacy-console behaviors K depends on (no Ctrl+V hijacking, native mouse-wheel scrolling, full-screen console resize, honor of per-EXE "Use legacy console" / QuickEdit registry settings).  Window-visibility is used rather than `WT_SESSION` because Win11's "default-terminal" routing -- by which WT can host consoles spawned by unrelated parents like Total Commander -- does not propagate `WT_SESSION` into the child process environment.  Set environment variable `K_NO_CONHOST_RELAUNCH=1` to disable the relaunch (e.g. for debugging k.exe under WT).
