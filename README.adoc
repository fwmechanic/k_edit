 ...
 * As of 2026 the build uses https://www.msys2.org/[MSYS2] UCRT64.  (Previously http://nuwen.net/mingw.html[nuwen.net MinGW]; the transition was driven by nuwen dropping PCRE2 in late 2025.)
...
2026/04: Migrated the Windows toolchain from http://nuwen.net/mingw.html[nuwen.net MinGW] to https://www.msys2.org/[MSYS2] UCRT64.  nuwen dropped PCRE2 in late 2025, and since PCRE2 is central to K's regex functionality, continuing with nuwen was untenable.  The switch to UCRT64 also brings K from the 1990s-era `msvcrt.dll` to the modern `ucrtbase.dll` (UCRT, shipped with every Windows 10+ install), with a new runtime dep on `libwinpthread-1.dll` from GCC's POSIX threading model.
