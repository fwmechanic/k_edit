...
void Win::Maximize() {
   ...
   { Point tmp{ .lin=100, .col=100 }; SizePct_set( tmp ); }
   // DBG( "%s", __func__ );
   }
...
void Wins_WriteStateFile( FILE *ofh ) {
   ...
   while( 1 ) {
      ...
      for( auto ix(0) ; ix < hdsMax; ++ix ) {
         hds[ix].ToSaveCand();
         if( hds[ix].View() ) { DV && DBG("hds[%d] %8" PR_TIMET "d %s", ix, hds[ix].View()->TmFocusedOn(), hds[ix].View()->FBuf()->Name() );
      ...
      DV && DBG("hds[%d] %8" PR_TIMET "d %s  *** SAVING ***", best.get_idx(), pv->TmFocusedOn(), pv->FBuf()->Name() );
