...
Path::str_t Path::Absolutize( PCChar pszFilename ) {  enum { SD = 0 };
   ...
   try {
      const auto src( std::filesystem::absolute( std::filesystem::path( pszFilename ) ) ); // src = absolutized copy of pszFilename
      ...
      for( ; it != src.end() ; ++it ) {
         ...
         if( !std::filesystem::exists( appendNxt ) ) { break; }
         rv = appendNxt;
         }                                        SD && DBG( "%s +can'%s'", __func__, rv.c_str() );
      rv = std::filesystem::canonical(rv);        SD && DBG( "%s -can'%s'", __func__, rv.c_str() );
...
//    Path::str_t dests ( rv.string()         );  SD && DBG( "%s s  '%s' -> '%s'", __func__, pszFilename, dests .c_str() );
      ...
      Msg( "%s caught std::filesystem::filesystem_error on %s", __func__, pszFilename );
      return "";
      };
   }
