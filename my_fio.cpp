...
int fio::Write( int fh, PCVoid pBuf, size_t bytesToWrite ) {
   auto rv( WL( _write, write )( fh, pBuf, bytesToWrite ) );  0 && DBG( "%s [%d]: %" PR_SIZET "u -> %" PR_SIZET /*BOGUS*/ "d", __func__, fh, bytesToWrite, rv );
   if( rv == -1 )
       rv = 0;
   return rv;
   }
...
bool unlinkOk_( PCChar filename, PCChar caller ) {
   ...
   if( err ) {      DBG( "!!! unlink (by %s) of '%s' failed, emsg='%s'", caller, filename, strerror( errno ) ); }
