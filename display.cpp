...
STATIC_CONST struct
   ...
   {
    { U8('≈'), U8('¥'), U8('√'), U8('≥'), U8('ƒ'), U8('¡'), U8('¬') },  // 0
    { U8('◊'), U8('∂'), U8('«'), U8('∫'), U8('ƒ'), U8('–'), U8('“') },  // 1
    { U8('ÿ'), U8('µ'), U8('∆'), U8('≥'), U8('Õ'), U8('œ'), U8('—') },  // 2
    { U8('Œ'), U8('π'), U8('Ã'), U8('∫'), U8('Õ'), U8(' '), U8('À') },  // 3
...
#elif  (1==LINEDRAW_WIN)
                         HV_=U8('◊'), LV_=U8('∂'), RV_=U8('«'), _V_=U8('∫'), H__=U8('ƒ'), HT_=U8('–'), HB_=U8('“'),  // "÷ƒ∑”Ω∫«∂“–◊",
#elif  (2==LINEDRAW_WIN)
                         HV_=U8('ÿ'), LV_=U8('µ'), RV_=U8('∆'), _V_=U8('≥'), H__=U8('Õ'), HT_=U8('œ'), HB_=U8('—'),  // "’Õ∏‘æ≥∆µ—œÿ"
#elif  (3==LINEDRAW_WIN)
                         HV_=U8('Œ'), LV_=U8('π'), RV_=U8('Ã'), _V_=U8('∫'), H__=U8('Õ'), HT_=U8(' '), HB_=U8('À'),  // "…Õª»º∫ÃπÀ Œ",
#elif  (4==LINEDRAW_WIN)
...
STATIC_FXN void DeleteFES( void *pData, void *pExtra ) {
   auto pFES( static_cast<PFileExtensionSetting>(pData) );
   Free0( pFES );
   }
...
STATIC_FXN COL GetLineForDisplay( const LINE yDisplayLine, Linebuf &DestLineBuf, LineColors &alc, PCHiLiteRec &pFirstPossibleHiLite ) {
   ...
   memset( PChar(DestLineBuf), H__, scrnCols ); //***** initial assumption: this line is a horizontal border ('Õ')
   ...
   if( WithinRangeInclusive( d_UpLeft.lin, yLineOfDisplay, d_UpLeft.lin + d_Size.lin - 1 ) ) {
      ...
      if( d_UpLeft.col > 0 ) { // this window not on left edge? (i.e. window has visible left border?) plug in a line-draw char to make the border
         ...
         if     ( chLeftBorder == H__                               // 'Õ' -> 'π'
                ||chLeftBorder == HT_                               // ' ' -> 'π'
                ||chLeftBorder == HV_ ) { chLeftBorder = U8(LV_); } // 'Œ' -> 'π'
         else if( chLeftBorder == RV_ ) { chLeftBorder = U8(_V_); } // 'Ã' -> '∫'
         }

      auto    &chRightBorder( destLineBuf[ oRightBorder ] );
      if     ( chRightBorder == H__                                  // 'Õ' -> 'Ã'
             ||chRightBorder == HV_ ) { chRightBorder = U8(RV_); }   // 'Œ' -> 'Ã'
      else if( chRightBorder == LV_ ) { chRightBorder = U8(_V_); }   // 'π' -> '∫'
      }
   else if( yLineOfDisplay == d_UpLeft.lin - 1 ) {  // window's top border line?
      auto    &chRightBorder( destLineBuf[ oRightBorder ] );
      if     ( chRightBorder == H__ ) { chRightBorder = U8(HB_); }   // 'Õ' -> 'À'
      else if( chRightBorder == HT_ ) { chRightBorder = U8(HV_); }   // ' ' -> 'Œ'
      }
   else if( yLineOfDisplay == d_UpLeft.lin + d_Size.lin ) { // window's bottom border line?
      auto    &chRightBorder( destLineBuf[ oRightBorder ] );
      if     ( chRightBorder == H__ ) { chRightBorder = U8(HT_); }   // Õ ->  
      else if( chRightBorder == HB_ ) { chRightBorder = U8(HV_); }   // À -> Œ
      }
   }
